...
5. 在 `server.py` 的 `MODULE_CATALOG`、默认用户类型权限和 `route_module()` 中注册；
6. 为访客开放时同步更新后端 `PUBLIC_MODULES` 和前端 `guestPages`。
...
新增模块时应同时定义 `view/create/edit/delete` 的默认权限。管理员切换用户视图只影响前端展示，服务端仍识别管理员账号，因此危险操作必须有显式确认。
